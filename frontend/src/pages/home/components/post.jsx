...
import Comments from "./Comments"; // Adjust the path if necessary

function Post() {
  // State to control the display of the comments box
    ...
    <div className="post-header flex w-full flex-row">
      <div className="flex basis-1/2 gap-2">
        ...
        <div className="flex flex-col justify-center">
          ...
          <span className="font-light text-xs opacity-75">30 mins ago</span>
        </div>
      </div>
      <div className="flex basis-2/3 flex-row justify-end">
        <Badge
          className="rounded-[20px] border-none bg-cyan-600 px-5 font-light text-white"
          variant="outline"
        >
          #Volunteer
        </Badge>
  ...
  const PostBody = () => (
    <div className="post-body">
      ...
      <span className="mt-3 mb-6 block leading-[1.5] font-light">
        ...
        experience, and connect with amazing people? Join us for our upcoming
        event!
      </span>
      ...
      <div className="flex flex-row items-center gap-x-2">
        <Map className="opacity-50" size={24} />
        <span className="text-base font-light opacity-65">Main Campus</span>
      </div>
    </div>
  );

  // Define PostFooter inline with a button that shows the comment box
  const PostFooter = () => (
    <div>
      <Separator className="mt-6" />
      <div className="columns-2 divide-gray-100 py-2 text-center text-gray-500">
        <div
          className="hover-effect flex justify-center gap-2 rounded-md py-2 cursor-pointer"
